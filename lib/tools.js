...
const WINDOW = "Controls background tabs in the user's current Google Chrome, inside a collapsed DSH tab group. Does not open a separate window and does not steal focus. Uses that Chrome profile, including its logins. Only http and https URLs are allowed. Requires the DSH Chrome extension.";
...
export function chromeTools() {
  return [
    ...
    tool(
      "chrome_close",
      `${WINDOW} Close the DSH tab group. Does not quit Chrome or close the user's other tabs.`,
      {},
      [],
      15_000,
