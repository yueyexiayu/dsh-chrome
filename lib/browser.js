...
async function ensure(signal) {
  ...
  session = null;
  const cdp = new Cdp(await connectBridge(signal));
...
export function downloads(signal) {
  return exclusive(async (active) => {
    const current = await ensure(active);
    const listed = await current.cdp.send("Browser.getDownloadItems", {}, undefined, active).catch(() => null);
...
export function browsers(action) {
  ...
  if (name === "list") {
    return Promise.resolve([
      ...
      session && !session.cdp.closed ? "bridge: connected" : "bridge: not connected",
    ].join("\n"));
  }
...
export function closeBrowser(signal) {
  return exclusive(async () => {
    await shutdown();
    return "closed the DSH tab group. Chrome stays open.";
  }, signal);
}
