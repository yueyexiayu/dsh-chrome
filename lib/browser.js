/**
 * Launch a dedicated visible Chrome and drive one page over CDP.
 * This is not the user's everyday Chrome profile.
 */
...
export const DEBUG_PORT = Number(process.env.CHROME_DEBUG_PORT || 9333);
let activePort = DEBUG_PORT;
...
async function chromeBinary() {
  ...
  throw new Error("Google Chrome was not found. Set CHROME_PATH to the Chrome executable.");
}
...
async function waitForVersion(port, signal) {
  const deadline = Date.now() + LAUNCH_MS;
  let last = "not up";
  while (Date.now() < deadline) {
    if (signal?.aborted) throw new Error("cancelled");
    try {
      return await readVersion(port, signal);
    } catch (error) {
      last = error instanceof Error ? error.message : String(error);
    }
    await delay(200, signal);
  }
  throw new Error(`Chrome debug port ${port} did not open: ${last}`);
}

function userDataDir() {
  const dir = path.join(dshHome(), "cache", "chrome-control");
  mkdirSync(dir, { recursive: true });
  return dir;
}

async function launchChrome(port, signal) {
  ...
  child.on("error", () => {});
  const version = await waitForVersion(port, signal);
...
function connectSocket(url) {
  ...
  return new Promise((resolve, reject) => {
    ...
    ws.addEventListener("error", () => {
      clearTimeout(timer);
      reject(new Error("failed to connect to Chrome"));
    }, { once: true });
  });
}

function downloadDir() {
  const dir = path.join(dshHome(), "cache", "chrome-downloads");
  mkdirSync(dir, { recursive: true });
  return dir;
}
...
function onBrowserEvent(message) {
  ...
  if (message.method === "Browser.downloadWillBegin" || message.method === "Browser.downloadProgress" || message.method === "Page.downloadWillBegin" || message.method === "Page.downloadProgress") {
    noteDownload(session, message.params || {});
    return;
  }
  if (message.sessionId && message.sessionId !== session.sessionId) return;
  if (message.method === "Runtime.executionContextCreated") {
    noteContext(session, message.params?.context);
    return;
  }
  if (message.method === "Runtime.executionContextDestroyed") {
    session.contexts?.delete(message.params?.executionContextId);
    return;
  }
  ...
  if (message.method === "Runtime.consoleAPICalled") {
    pushLimited(session.console, { level: message.params?.type || "log", text: consoleText(message.params || {}) }, CONSOLE_LIMIT);
    ...
    noteRequest(session, message.params || {});
  } else if (message.method === "Network.responseReceived") {
    const id = message.params?.requestId;
    const row = id && session.network.get(id);
    if (row) {
...
async function ensure(signal) {
  if (session && !session.cdp.closed && session.sessionId) return session;
  session = null;
  let version;
  try {
    version = await readVersion(activePort, signal);
  } catch {
    const launched = await launchChrome(activePort, signal);
    launchedByUs = true;
    version = launched.version;
  }
  const socketUrl = version.webSocketDebuggerUrl;
  if (!socketUrl) throw new Error("Chrome did not provide a DevTools websocket");
  const cdp = await connectSocket(socketUrl);
  await cdp.send("Target.setDiscoverTargets", { discover: true }, undefined, signal);
  await cdp.send("Browser.setDownloadBehavior", {
    behavior: "allow",
    downloadPath: downloadDir(),
    eventsEnabled: true,
  }, undefined, signal).catch(() => {});
  const page = await ensurePage(cdp, signal);
  session = {
    cdp,
    ...page,
    dialog: "",
  ...
  launchedByUs = false;
  cdp.listeners.add(onBrowserEvent);
  await cdp.send("Runtime.disable", {}, session.sessionId, signal).catch(() => {});
  await cdp.send("Runtime.enable", {}, session.sessionId, signal);
...
async function ensurePage(cdp, signal, targetId) {
  const listed = await cdp.send("Target.getTargets", {}, undefined, signal);
  ...
  if (!target) {
    const created = await cdp.send("Target.createTarget", { url: "about:blank" }, undefined, signal);
  ...
  await cdp.send("Page.bringToFront", {}, sessionId, signal).catch(() => {});
...
export function navigate(url, options = {}) {
  ...
  return exclusive(async (signal) => {
    ...
    if (options.newTab) {
      const created = await current.cdp.send("Target.createTarget", { url: "about:blank" }, undefined, signal);
      const page = await ensurePage(current.cdp, signal, created.targetId);
...
export function tabs(action, targetId, signal) {
  ...
  return exclusive(async (active) => {
    const current = await ensure(active);
    if (name === "list") {
      const listed = await current.cdp.send("Target.getTargets", {}, undefined, active);
...
export function downloads(signal) {
  return exclusive(async (active) => {
    const current = await ensure(active);
    return formatDownloads(current.downloads, downloadDir());
  }, signal);
}
...
export function resize(width, height, signal) {
  ...
  return exclusive(async (active) => {
    ...
    await current.cdp.send("Browser.setWindowBounds", {
      windowId: found.windowId,
      bounds: { width: Math.round(w), height: Math.round(h), windowState: "normal" },
    }, undefined, active);
    await delay(200, active);
    return `window: ${Math.round(w)}x${Math.round(h)}\n${await observe(current, active)}`;
  }, signal);
}
 ...
 * @returns {Promise<string>}
 */
export function browsers(action, port, signal) {
  const name = String(action || "list");
  if (name === "list") {
    return Promise.resolve([
      `port: ${activePort}`,
      `launched: ${session?.launched ? "yes" : "attached"}`,
      "attach with chrome_browsers action=attach and a debug port to switch Chrome instances.",
    ].join("\n"));
  }
  if (name !== "attach") throw new Error("action must be list or attach");
  const next = Number(port);
  if (!Number.isInteger(next) || next < 1 || next > 65535) throw new Error("port must be a TCP port");
  if (session && !session.cdp.closed) session.cdp.close();
  session = null;
  activePort = next;
  launchedByUs = false;
  return snapshot(signal).then((text) => `attached ${next}\n${text}`);
}
...
export function closeBrowser(signal) {
  return exclusive(async () => {
    await shutdown();
    return "closed the controlled Chrome window";
  }, signal);
}

/** Close the controlled Chrome. Tests use this; the plugin leaves the window open. */
export async function shutdown() {
  const current = session;
  session = null;
  if (!current || current.cdp.closed) return;
  try {
    await current.cdp.send("Browser.close", {}, undefined, undefined, 3000);
  } catch {
    // already gone
  }
  current.cdp.close();
